...
class FullScreenScaleMode extends flixel.system.scaleModes.BaseScaleMode
{
   ...
   */
  public static var maxRatioAxis:FlxAxes = FlxAxes.X;
   ...
   */
  public static var ratioAxis:FlxAxes = FlxAxes.X;
  ...
  function updateDeviceCutout(Width:Int, Height:Int):Void
  {
      ...
      cutoutSize.x = ratioAxis == FlxAxes.X ? Math.ceil(Width - logicalSize.x) : 0;
      cutoutSize.y = ratioAxis == FlxAxes.Y ? Math.ceil(Height - logicalSize.y) : 0;
  ...
  private function adjustGameSize():Void
  {
    if ((cutoutSize.x > 0 || cutoutSize.y > 0) && enabled)
    {
      ...
      if (ratioAxis == Y)
      {
        ...
        FlxG.height = Math.ceil(gameHeight);
      ...
      else
      {
        ...
        FlxG.width = Math.ceil(gameWidth);
