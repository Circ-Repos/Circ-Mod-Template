...
void main() {
    ...
    if (all(lessThanEqual(uv, vec2(1.0))))
    {
        ...
        vec3 yuv[4];
        yuv[0] = rgbToYuv(texture2D(bitmap, blockCoord + vec2(-1.5, 0.5) / openfl_TextureSize).rgb);
        yuv[1] = rgbToYuv(texture2D(bitmap, blockCoord + vec2(-0.5, 0.5) / openfl_TextureSize).rgb);
        yuv[2] = rgbToYuv(texture2D(bitmap, blockCoord + vec2(-1.5, 1.5) / openfl_TextureSize).rgb);
        yuv[3] = rgbToYuv(texture2D(bitmap, blockCoord + vec2(-0.5, 1.5) / openfl_TextureSize).rgb);
        ...
        vec3 UV_avg = mix(
            ...
            mix(yuv[2], yuv[3], fuv.x), vec3(fuv.y)
        );

        float Y_current = rgbToYuv(texture2D(bitmap, openfl_TextureCoordv).rgb).x;

        gl_FragColor = vec4(clamp(yuvToRgb(Y_current, UV_avg.y, UV_avg.z), 0.0, 1.0), 1.0);
    }
}